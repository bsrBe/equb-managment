...
function buildDataSourceOptions(): DataSourceOptions {
  // Neon (remote) connection – use the full URL
  if (process.env.DATABASE_URL) {
    return {
      type: 'postgres',
      url: process.env.DATABASE_URL,
      // Neon requires TLS; the default client will negotiate it.
      // If you need to disable verification (not recommended), uncomment:
      ssl: { rejectUnauthorized: false },
      entities: [Admin, User, Equb, Attendance, EqubMember, Payout, Period],
      synchronize: true,
    };
